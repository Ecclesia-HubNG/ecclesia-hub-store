      ...
      {open && createPortal(
        <>
          ...
          <button
            ...
            onClick={() => {
              setOpen(false)
              startTransition(() => setUserRole(user.id, isAdmin ? 'user' : 'admin'))
            }}
            className="flex items-center gap-2.5 w-full px-3.5 py-2.5 text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors"
          >
            <svg className="w-4 h-4 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
            ...
            Make {isAdmin ? 'regular user' : 'admin'}
          </button>
          ...
          <button
            ...
            onClick={() => {
              setOpen(false)
              startTransition(() => setUserBanned(user.id, !banned))
            }}
...
export default function UsersManager({ users: initial }: { users: AuthUser[] }) {
  ...
  const stats = [
    ...
    { label: 'Admins', value: initial.filter(u => u.app_metadata?.role === 'admin').length, accent: true },
  ...
  const roleLabels: Record<RoleFilter, string> = { all: 'All roles', admin: 'Admin', user: 'User' }
          ...
          {showRoleDd && (
            <div className="absolute left-0 top-full mt-1 z-20 w-36 bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl shadow-lg py-1">
              {(['all', 'admin', 'user'] as RoleFilter[]).map(r => (
                <button key={r} type="button" onClick={() => { setRoleFilter(r); setShowRoleDd(false) }}
                  className={`flex items-center justify-between w-full px-3 py-2.5 text-sm transition-colors ${roleFilter === r ? 'text-gray-900 dark:text-white font-medium bg-gray-50 dark:bg-gray-800' : 'text-gray-600 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-800'}`}>
                  {roleLabels[r]}
                  {roleFilter === r && <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}><path strokeLinecap="round" strokeLinejoin="round" d="m4.5 12.75 6 6 9-13.5" /></svg>}
                </button>
              ))}
            </div>
          )}
                  ...
                  <tr key={user.id} className={`transition-colors hover:bg-gray-50 dark:hover:bg-gray-800/40 ${banned ? 'opacity-60' : ''}`}>
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-3">
                        <div className={`w-9 h-9 rounded-full flex items-center justify-center shrink-0 text-xs font-bold ${isAdmin ? 'bg-[#4A0F1C] text-white' : 'bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-300'}`}>
                          {getInitials(name)}
                        </div>
                        <div>
                    ...
                    <td className="px-4 py-3">
                      <span className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold ${
                        isAdmin ? 'bg-[#4A0F1C]/10 dark:bg-[#4A0F1C]/30 text-[#6B1A2A] dark:text-[#D4849A]' : 'bg-gray-100 dark:bg-gray-800 text-gray-500 dark:text-gray-400'
                      }`}>
                        {isAdmin ? 'Admin' : 'User'}
                      </span>
                    </td>
                    ...
                    <td className="px-3 py-3 text-right">
                      <ActionMenu user={user} onEdit={() => setEditUser(user)} onDelete={() => setDeleteTarget(user)} />
                    </td>
                  </tr>
                )
              })}
