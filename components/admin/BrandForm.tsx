...
import BrandsSideList from '@/components/admin/BrandsSideList'
...
export default function BrandForm({
  ...
  const uploadFile = async (file: File) => {
    ...
    setUploading(false)
    if (!res.ok) { setUploadError(json.error ?? 'Upload failed'); setLogoPreview(null); return }
    setLogoUrl(json.url)
  ...
  const hasRightCol = brands !== undefined
      ...
      <div className={`grid gap-6 ${hasRightCol ? 'grid-cols-1 xl:grid-cols-[minmax(0,1fr)_340px]' : 'grid-cols-1 max-w-2xl'}`}>
        ...
        <div className="space-y-5 min-w-0">
          ...
          <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl p-5">
            <h2 className="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-4">Brand Logo</h2>

            {logoPreview ? (
              <div className="relative group">
                <div className="w-full h-40 rounded-xl overflow-hidden bg-gray-50 dark:bg-gray-800 flex items-center justify-center">
                  <img src={logoPreview} alt="" className="max-h-32 max-w-full object-contain" />
                  {uploading && (
                ...
                <div className="absolute top-2 right-2 flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                  ...
                  <button type="button" onClick={() => { setLogoPreview(null); setLogoUrl(null) }}
              ...
              >
                ...
                <p className="text-sm font-medium text-gray-600 dark:text-gray-400">
                  Drop logo here, or <span className="text-gray-900 dark:text-white underline underline-offset-2">browse</span>
                </p>
                <p className="text-xs text-gray-400 dark:text-gray-600">PNG, JPG, SVG, WebP up to 10 MB</p>
              </div>
            )}
          ...
          <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl p-5 space-y-4">
            ...
            <div>
              ...
              <input name="name" value={name} onChange={e => setName(e.target.value)} required placeholder="e.g. Crossway" className={inputCls} />
            </div>

            <div>
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">URL Slug</label>
              <input name="slug" value={slug} onChange={e => { setSlug(e.target.value); setSlugEdited(true) }} required placeholder="crossway" className={`${inputCls} font-mono`} />
            ...
            <div>
              ...
              <textarea name="description" value={description} onChange={e => setDescription(e.target.value)} rows={3} placeholder="A short description of the brand…" className={`${inputCls} resize-none`} />
            </div>

            <div>
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">
                Website <span className="text-xs font-normal text-gray-400 dark:text-gray-600">(optional)</span>
              </label>
              <input name="website" type="url" value={website} onChange={e => setWebsite(e.target.value)} placeholder="https://crossway.org" className={inputCls} />
            </div>
          </div>
          ...
          <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl p-5">
            ...
            <label className="flex items-center gap-3 cursor-pointer">
              ...
              <div>
                <p className="text-sm font-medium text-gray-700 dark:text-gray-300">Featured brand</p>
                <p className="text-xs text-gray-400 dark:text-gray-600">Highlight this brand on the homepage and shop page</p>
              </div>
            </label>
          </div>
          ...
          <div className="flex items-center gap-3 pb-8">
            <button type="submit" disabled={isPending || uploading}
              ...
              {uploading ? 'Uploading logo…' : isPending ? 'Saving…' : submitLabel}
            </button>
            <Link href="/admin/brands" className="px-5 py-2.5 text-sm text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white transition-colors">
              Cancel
            </Link>
          </div>
        </div>

        {/* Right — brands list (new page only) */}
        {hasRightCol && (
          <div className="min-w-0">
            <BrandsSideList brands={brands} />
          </div>
        )}
      </div>
    </form>
