    ...
    <div className="bg-gray-50 dark:bg-gray-800/50 rounded-2xl p-5 space-y-4">
      <div className="grid grid-cols-2 gap-4">
        <div className="col-span-2">
          <label className="block text-xs font-medium text-gray-600 dark:text-gray-400 mb-1.5">Display name <span className="text-red-500">*</span></label>
          <input
            value={form.name}
            ...
            placeholder="e.g. Standard Delivery — Lagos, Express — Lekki"
          />
        </div>
        <div>
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-0.5">
            Define the delivery options customers see at checkout. Orders only ship to locations you add here.
          </p>
        </div>
        {!showAdd && (
          <button
      ...
      {zones.length === 0 && !showAdd ? (
        <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-100 dark:border-gray-800 p-12 text-center">
          ...
          <p className="text-sm font-medium text-gray-900 dark:text-white mb-1">No shipping options yet</p>
          <p className="text-xs text-gray-400">Add options to let customers select a delivery method at checkout</p>
        </div>
      ) : (
              ...
              {editId === zone.id ? (
                ...
                <div className="flex items-center px-5 py-3.5 gap-4">
                  <div className="flex-1 min-w-0">
                    <p className={`text-sm font-medium ${zone.is_active ? 'text-gray-900 dark:text-white' : 'text-gray-400'}`}>
                      {zone.name}
                    </p>
                    <p className="text-xs text-gray-400 mt-0.5">
                      {zone.state}{zone.area ? ` · ${zone.area}` : ''}
                      {!zone.is_active && <span className="ml-2 font-semibold uppercase tracking-wide text-[10px]">Disabled</span>}
                    </p>
                  </div>
      ...
      <div className="bg-blue-50 dark:bg-blue-900/10 border border-blue-100 dark:border-blue-900/30 rounded-xl px-5 py-4">
        <p className="text-xs font-semibold text-blue-800 dark:text-blue-300 mb-1">How shipping options work</p>
        <ul className="text-xs text-blue-700 dark:text-blue-400 space-y-1 list-disc list-inside">
          <li>Each option you add here appears as a choice in the checkout dropdown.</li>
          <li>Give each option a clear <strong>Display name</strong> customers will recognise — e.g. "Standard Delivery – Lagos" or "Express – Lekki".</li>
          <li>Only active options are shown at checkout. Disable any you don't currently offer.</li>
          <li>Orders can only be placed to locations you define here.</li>
        </ul>
      </div>
    </div>
  )
