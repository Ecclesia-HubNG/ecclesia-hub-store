...
import { setNewArrival } from '@/lib/actions/products'
...
export default function NewArrivalsManager({ initialProducts }: { initialProducts: Product[] }) {
  ...
  const added = useMemo(() => products.filter(p => p.is_new_arrival), [products])
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-center gap-4 px-5 py-3.5 bg-gray-50 dark:bg-gray-800/50 rounded-xl border border-gray-200 dark:border-gray-700">
        <div>
          <p className="text-2xl font-bold text-gray-900 dark:text-white">{added.length}</p>
          <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">New Arrivals</p>
        </div>
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="relative flex-1">
          ...
          <input
            type="text"
            placeholder={`Search ${tab === 'added' ? 'added' : 'all'} products…`}
            value={search}
              ...
              return (
                <div key={product.id} className="flex items-center gap-3 px-4 py-3">
                  ...
                  {product.is_new_arrival && (
                    <span className="px-2 py-0.5 text-[10px] font-bold bg-emerald-50 dark:bg-emerald-950/40 text-emerald-700 dark:text-emerald-400 border border-emerald-200 dark:border-emerald-800 rounded-full shrink-0">
                      NEW
                    </span>
                  )}
                  <button
                    type="button"
                    disabled={isPending}
                    onClick={() => toggle(product.id, product.is_new_arrival)}
                  ...
                  >
                    {isPending ? '…' : product.is_new_arrival ? 'Remove' : 'Add'}
                  </button>
                </div>
              )
            })}
